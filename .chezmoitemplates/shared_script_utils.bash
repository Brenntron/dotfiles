...
# Set Options
# ######################
shopt -s nullglob                                    # Safe on Bash 3.2+; make `for f in *.txt` work when `*.txt` matches zero files
[ "${BASH_VERSINFO:-0}" -ge 4 ] && shopt -s globstar # globstar requires Bash 4+
...
_printFuncStack_() {
  ...
  for ((_i = 1; _i < ${#BASH_SOURCE[@]}; _i++)); do
    ...
    _alert_ | _trapCleanup_ | fatal | error | warning | notice | info | debug | dryrun | header | success)
      ...
      ;;
    *)
      _funcStackResponse=("${_funcStackResponse[@]}" "${FUNCNAME[${_i}]}:$(basename "${BASH_SOURCE[${_i}]}"):${BASH_LINENO[_i - 1]}")
      ;;
    esac
