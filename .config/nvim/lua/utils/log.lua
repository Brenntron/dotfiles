...
function Log:set_level(level)
  if
    not pcall(function()
      local logger_ok, logger = pcall(function()
        return require("structlog").get_logger "Brenntron"
      end)
  ...
  then
    vim.notify "Structlog version is too old."
  end
end
...
function Log:init()
      ...
      pipelines = {
        {
          ...
          sink = structlog.sinks.Console(false),
        },
        {
          level = log_level,
          processors = {
  ...
  local logger = structlog.get_logger "Brenntron"
...
function Log:get_path()
  return string.format("%s/%s.log", get_cache_dir(), "Brenntron")
end
