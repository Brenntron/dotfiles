          ...
          resource "escalations/file_rep/sample_zoo" do
            ...
            get ":sha256_hash" do
              if /\A\H*(?<sha256_hash>\h{64})\H*\z/ =~ permitted_params[:sha256_hash]
                api_response = FileReputationApi::SampleZoo.query_from_data(api_response)
                begin
                  # no need to pluck, since api_response is already trimmed down to what we need?
                  attributes = api_response
                  FileReputationDispute.where(sha256_hash: sha256_hash).update_all(api_response)
                rescue => except
