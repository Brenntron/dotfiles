      ...
      if self.is_important
        if self.status == "PENDING"
          if commit_pending == "commit"
            ...
            update(status:current_status,internal_comment: comment, case_resolved_at: Time.now,user:current_user)
            ...
            update(status:current_status, internal_comment: comment, case_assigned_at: Time.now)
          end
        else
          current_status = "PENDING"
          update(resolution:entry_status,url_primary_category:categories_string,category:categories_string,status:current_status,internal_comment: comment,user:current_user)
        end
      else
        current_status = "COMPLETED"
        self.case_assigned_at ||= Time.now
        update(resolution:entry_status,url_primary_category:categories_string,category:categories_string,status:current_status,internal_comment: comment, case_resolved_at: Time.now,user:current_user)
        complaint.set_status(current_status)
        #this is where we should send off the category to the API
        commit_category(ip_or_uri: self.uri_or_ip, categories_string: categories_string, description: comment, user: current_user.email)
