class DisputeEmail < ApplicationRecord
  ...
  def self.process_bridge_payload(message_payload)
    begin
      ...
      conn = ::Bridge::EmailFailedEvent.new(addressee: "talos-intelligence", source_authority: "talos-intelligence", source_key: message_payload["source_key"])
      conn.post()
    end
  end
  ...
  def self.generate_case_email_address(dispute_id)
    # Replace email_user with below line once we have confirmed it sends properly through the bridge
