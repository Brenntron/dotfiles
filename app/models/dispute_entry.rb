class DisputeEntry < ApplicationRecord
  ...
  def get_xbrs_value
    ...
    case
    when self.entry_type == "IP"
      xbrs = Xbrs::GetXbrs.by_ip4(self.ip_address)
    when self.entry_type == "URI/DOMAIN"
      xbrs = Xbrs::GetXbrs.by_domain(self.uri)
    else
      self.uri.blank? ? xbrs = Xbrs::GetXbrs.by_ip4(self.ip_address) : xbrs = Xbrs::GetXbrs.by_domain(self.uri)
    end
