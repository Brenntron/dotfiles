class ReverseLabsApi < ApplicationRecord

  def self.host
    Rails.configuration.reverselabs.host
  end
  #
  ...
  def self.query_lookup(params, retries = nil)
    ...
    begin
      json_response = self.get_auth_key(Rails.configuration.reverselabs.user,Rails.configuration.reverselabs.pass, params["retried"])
...
#
# def self.reversing_labs_lookup(sha256)
#   uri = URI("https://ticloud-cdn-api.reversinglabs.com/api/databrowser/rldata/query/sha256/#{sha256.sha256}?format=json")
#
#   if Rails.cache.read(uri).blank?
#     request = Net::HTTP::Get.new(uri)
#     request.basic_auth ENV['RL_USER'], ENV['RL_PASS']
#     req_options = {
#         use_ssl: uri.scheme == "https",
...
#         filtered_response = ShaSearch.reversing_labs_filter(response)
#         Rails.cache.write(uri, filtered_response)
#         filtered_response
