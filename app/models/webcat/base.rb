class Webcat::Base
  ...
  def self.host
    @host ||= Rails.configuration.guard_rails.host || 'localhost'
  end

  def self.port
    @port ||= Rails.configuration.guard_rails.port || 443
  end

  def self.verify_mode
    @verify_mode ||= Rails.configuration.guard_rails.verify_mode
  end

  def self.gssnegotiate?
    @gssnegotiate ||= Rails.configuration.guard_rails.gssnegotiate
  end

  def self.ca_cert_file
    @ca_cert_file ||= Rails.configuration.guard_rails.ca_cert_file
  end

  def self.read_timeout
    @read_timeout ||= Rails.configuration.guard_rails.read_timeout
  end
  def self.open_timeout
    @open_timeout ||= Rails.configuration.guard_rails.open_timeout
  end
