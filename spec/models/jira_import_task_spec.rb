  ...
  describe '.export_xlsx' do
    ...
    context 'when issue_keys parameter is empty' do
      it 'returns a sheet with all JiraImportTask records' do
        ...
        expect(worksheet.sheet_data[1][0].value).to eq jira_import_task.issue_key
        expect(worksheet.sheet_data[1][1].value).to eq import_url.submitted_url
        expect(worksheet.sheet_data[1][2].value).to eq complaint_entry.id
        expect(worksheet.sheet_data[1][3].value).to eq complaint_entry.status
        expect(worksheet.sheet_data[1][4].value).to eq complaint_entry.resolution
        expect(worksheet.sheet_data[1][5].value).to eq jira_import_task.issue_status
        expect(worksheet.sheet_data[1][6].value).to eq jira_import_task.issue_type
        expect(worksheet.sheet_data[1][7].value).to eq jira_import_task.issue_summary
        expect(worksheet.sheet_data[1][8].value).to eq jira_import_task.submitter
        expect(worksheet.sheet_data[1][9].value).to eq jira_import_task.issue_platform
        expect(worksheet.sheet_data[1][10].value).to eq jira_import_task.status
        expect(worksheet.sheet_data[1][11].value).to eq jira_import_task.imported_at.utc.iso8601
        expect(worksheet.sheet_data[1][12].value).to eq 'Inactive'
      end
      ...
      it 'write correct headers' do
        ...
        described_class::EXPORT_FIELD_NAMES.values.each_with_index do |field_name, index|
    ...
    context 'when issue_keys parameter is not empty' do
      ...
      it 'returns a sheet with only matching JiraImportTask records' do
        ...
        expect(worksheet.sheet_data[0][0].value).to eq 'Jira Ticket ID'
        expect(worksheet.sheet_data.size).to eq expected_rows_amount
      end
