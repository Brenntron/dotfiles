...
RSpec.describe "complaint bulk submit", type: :request do
  ...
  it 'should get domain history from provided domain/url' do
    vrt_incoming
    guest_company
    login_as(current_user, :scope => :user)

    params = {}
    params[:domain] = "1234computer.com"


    get '/escalations/api/v1/escalations/webcat/complaint_entries/get_domain_history', params: params
    ...
    #expect(json_body["data"]["created"]).to eql(1)
    #expect(json_body["data"]["completed"]).to eql(1)
  ...
  xit 'should bulk submit multiple entry with multiple category' do
    vrt_incoming
    guest_company
    login_as(current_user, :scope => :user)
